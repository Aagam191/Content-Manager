...
const personalityContext = {
   "Cricket Expert": "As a cricket expert posting highlights, your content should be exciting and informative, offering detailed analysis, game insights, and captivating highlights to engage cricket enthusiasts.",
...
function Chat() {
  ...
  useEffect(() => {
    async function fetchChats() {
      try {
        const response = await axios.get("http://localhost:8000/chat/get-chats/");
  ...
  // Function to generate personality-based answer and store chat
  async function generateAnswer(e) {
    setGeneratingAnswer(true);
    e.preventDefault();
    setAnswer("Loading your answer... \n It might take up to 10 seconds");
    ...
    try {
      const response = await axios.post(
        'https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyDAtSeBPurtc3LdzdQC7ZgPKxB0IsQR3NA',
        {
          contents: [{ parts: [{ text: fullPrompt }] }],
        }
      );
      ...
      // Save chat history to Django backend
      await axios.post("http://localhost:8000/chat/save-chat/", {
        question: question,
        answer: newAnswer,
      });

      // Fetch updated chat history
      const chatResponse = await axios.get("http://localhost:8000/chat/get-chats/");
      setChatHistory(chatResponse.data);

    } catch (error) {
      console.error("Error generating answer or saving chat:", error);
      setAnswer("Sorry - Something went wrong. Please try again!");
    }
    ...
    <>
      <div className="chatdiv">
        <form onSubmit={generateAnswer} className="chat-form">
          ...
          <button
            type="submit"
            className={`chatsubmit ${generatingAnswer ? "chatsubmit1" : ""}`}
                        disabled={generatingAnswer}
          >
            Generate answer
          </button>
        ...
        <div className="chat-history">
          {chatHistory.map((chat) => (
            ...
            <div key={chat.id} className="chat-history-item">
              ...
              <strong>Expert:</strong> {chat.answer}
              <br />
              <em>{new Date(chat.created_at).toLocaleString()}</em>
            </div>
            <hr />
